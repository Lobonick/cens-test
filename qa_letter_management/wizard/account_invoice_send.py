...
class AccountLetterSend(models.TransientModel):
	_name = 'account.letter.send'
	_inherits = {'mail.compose.message':'composer_id'}
	_description = 'Account Letter Send'

	is_email = fields.Boolean('Email', default=lambda self: self.env.company.invoice_is_email)
	invoice_without_email = fields.Text(compute='_compute_invoice_without_email', string='bills that will not be sent')
	...
	invoice_ids = fields.Many2many('letter.management', 'letter_management_account_letter_send_rel', string='letters')
	composer_id = fields.Many2one('mail.compose.message', string='Composer', required=True, ondelete='cascade')
	template_id = fields.Many2one(
		'mail.template', 'Use template', index=True,
		domain="[('model', '=', 'letter.management')]"
		)

	@api.model
	def default_get(self, fields):
		res = super(AccountLetterSend, self).default_get(fields)
		res_ids = self._context.get('active_ids')

		# invoices = self.env['letter.management'].browse(res_ids).filtered(lambda move: move.is_invoice(include_receipts=True))
		# if not invoices:
		#     raise UserError(_("You can only send bills of exchange."))
	...
	def _compute_invoice_without_email(self):
		for wizard in self:
			if wizard.is_email and len(wizard.invoice_ids) > 1:
				invoices = self.env['letter.management'].search([
