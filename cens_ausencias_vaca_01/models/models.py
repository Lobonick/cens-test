...
class HrEmployeeCustom(models.Model):
    ...
    def action_genera_calculo_vaca(self):
        ...
        w_file = "CENS-VACACIONES-CÁLCULO.xlsx"
        w_erro = ""
        ...
        try:
            ...
            sorted_employees = w_lote.sorted(key=lambda r: r.name)
            ...
            for employee in sorted_employees:
                ...
                worksheet.write(w_fila, 2, employee.x_studio_documento_identidad, cell_format_cent)         #-- DNI
                if (employee.job_id.name):
                    worksheet.write(w_fila, 3, employee.job_id.name, cell_format_cent)                      #-- Puesto Laboral
                if employee.x_studio_negocio_unidad.x_name :
                    w_dato = employee.x_studio_negocio_unidad.x_name                                        #-- Unidad Negocio
                    worksheet.write(w_fila, 4, w_dato, cell_format_fech)
                # -----------------------------------------------------------------------------------------
                # CALCULA DETALLE DE LO GOZADO      -   QUIQUE
                # -----------------------------------------------------------------------------------------
                w_fecha_ingr = employee.contract_id.x_studio_fecha_de_ingreso               #-- FECHA INGRESO
                w_tiene_cese = employee.contract_id.x_empleado_cesado
                w_fecha_actu = date.today()
                if w_tiene_cese:
                    w_fecha_fina = employee.contract_id.x_studio_fecha_de_cese              #-- FECHA CESE
                    w_statu_cont = "CESADO"
                else:
                ...
                # w_tramo_dias = (w_cant_dd * (2.5/30))
                w_tramo_mess = (w_cant_mm * 30)
                w_tramo_dias = (w_cant_dd)
                w_dias_acum  = w_tramo_mess + w_tramo_dias
                ...
                # ----------------------------------------
                # BUSCA VACACIONES GOZADAS DEL EMPLEADO
                # ----------------------------------------
                found_leave = self.env['hr.leave'].search([
                            ('employee_id', '=', w_employee_id),
                            ('state', 'in', ['draft', 'confirm', 'refuse', 'validate1', 'validate'])  
                        ])
                if found_leave:
                    w_ausencias_ordenadas = found_leave
                    # sorted(w_agrupa_ausencias, key=lambda x: x['ausencia_periodo'])
                    w_switch = 1
                    w_actual = "XX"
                    # ------------------------------------
                    # IMPRIME DATOS DE VACACIONES GOZADAS 
                    # ------------------------------------
                    for ausencia in w_ausencias_ordenadas:
                        w_fecha_from = ausencia.request_date_from
                        w_fecha_to   = ausencia.request_date_to
                        if self.fecha_esta_entre(w_fecha_ingr, w_fecha_from, w_fecha_to):

                            w_period = f"{ausencia.request_date_from.year-1}-{ausencia.request_date_from.year}"
                            if not w_actual == w_period :
                                w_actual = w_period
                                w_switch = (1 if w_switch == 0 else 0)

                            worksheet.write(w_fila, 17, ausencia.x_cens_codiden, cell_format_tiny)
                            if self.estatus_ausencia(ausencia.state)=="Aprobado":
                                w_dato = f"{ausencia.request_date_from.year-1}-{ausencia.request_date_from.year}"
                            else:
                                w_dato = "NONE"
                            worksheet.write(w_fila, 18, w_dato, cell_format_cent)
                            worksheet.write(w_fila, 19, ausencia.request_date_from if self.estatus_ausencia(ausencia.state)=="Aprobado" else '01/01/1900', cell_format_fech)
                            worksheet.write(w_fila, 20, ausencia.request_date_to if self.estatus_ausencia(ausencia.state)=="Aprobado" else '01/01/1900', cell_format_fech)
                            worksheet.write(w_fila, 21, ausencia.number_of_days, cell_format_nume)
                            w_dias_goza += ausencia.number_of_days
                            worksheet.write(w_fila, 22, self.estatus_ausencia(ausencia.state), cell_format_perd if ausencia.state == 'refuse' else cell_format_left)
                            if ausencia.name:
                                worksheet.write(w_fila, 23, ausencia.name, cell_format_left)
                            worksheet.write(w_fila, 24, ausencia.create_uid.name, cell_format_left)
                            worksheet.write(w_fila, 25, ausencia.create_date, cell_format_fech)
                            # ----------------------------------
                            ...
                            worksheet.conditional_format(f'A{w_fila+1}:K{w_fila+1}', {
                                    'type': 'formula',
                                    'criteria': f'NOT(ISBLANK($S${w_fila+1}))',  # Evalúa si columna "M" no está vacía #D8E4BC
                            ...
                            worksheet.conditional_format(f'R{w_fila+1}:X{w_fila+1}', {
                                    'type': 'formula',
                                    'criteria': f'NOT(ISBLANK($S${w_fila+1}))',  # Evalúa si M no está vacía
                    ...
                    worksheet.conditional_format(f'A{w_fila+1}:K{w_fila+1}', {
                            'type': 'formula',
                            'criteria': f'NOT(ISBLANK($Q${w_fila+1}))',  # Evalúa si columna "M" no está vacía #D8E4BC
                            'format': w_formato_colorfila
                        })
                    worksheet.conditional_format(f'L{w_fila+1}:Q{w_fila+1}', {
                                    'type': 'formula',
        ...
        attachment = self.env['ir.attachment'].create({
            'name': 'CENS-VACACIONES - Cálculo General.xlsx',
            'type': 'binary',
            'datas': file_data,
            'store_fname': 'CENS-VACACIONES - Cálculo General.xlsx',
